"""
Starter fabfile for deploying the wtfhack project.
...
# CHANGEME
env.hosts = ['user@wtfhack.example.com']
env.code_dir = '/srv/www/wtfhack'
env.project_dir = '/srv/www/wtfhack/wtfhack'
env.static_root = '/srv/www/wtfhack/static/'
env.virtualenv = '/srv/www/wtfhack/.virtualenv'
env.code_repo = 'git@github.com:user/wtfhack.git'
env.django_settings_module = 'wtfhack.settings'
...
def restart():
    ...
    with cd(env.code_dir):
        run("touch %s/wtfhack/wsgi.py" % env.code_dir)
...
def test(unit=1, integration=1, functional=1, selenium=0, all=0):
    ...
    Call it like so:
        ...
        >> fab test:all=1
    """
    command = './manage.py test -v 2 --settings=wtfhack.settings.test_settings'
    if all == 0:
        if int(unit) == 0:
        ...
        if int(selenium) == 1:
            command = './manage.py test -v 2 --settings=wtfhack.settings.test_settings wtfhack/functional_tests/selenium_tests.py'
        else:
